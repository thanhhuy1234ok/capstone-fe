import { createCourseAPI, getMajorAPI, updateCourseAPI } from "@/services/api";
...
interface IProps {
    ...
    refreshTable: () => void;
    setDataUpdate: (v: ICourse | null) => void;
    dataUpdate: ICourse | null;
}
...
const ModalCourse = (props: IProps) => {
    ...
    const submitCohort = async (valuesForm: any) => {
        ...
        if (dataUpdate?.id) {
            ...
            const res = await updateCourseAPI(+id, cohort);
            if (res.data) {
            ...
            const res = await createCourseAPI(cohort);
            if (res.data) {
